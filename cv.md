---
layout: default
title: "Download CV — Fred Egbuedike"
---

<div class="card">
  <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 20px;">
    <h2 style="margin: 0; border: none;">📄 Download CV</h2>
    <button onclick="window.print()" style="background: #2e7d32; color: white; padding: 10px 20px; border: none; border-radius: 6px; cursor: pointer; font-weight: 600;">
      Print / Save as PDF
    </button>
  </div>
  <p style="color: var(--text-light);">This page is formatted for A4/letter printing. Click the button above to save as PDF.</p>
</div>

<div class="card" id="cv-content">
  <div style="text-align: center; margin-bottom: 24px; padding-bottom: 16px; border-bottom: 2px solid var(--primary);">
    <h1 style="color: var(--primary); font-size: 1.8rem; margin-bottom: 4px;">Fred Egbuedike</h1>
    <p style="color: var(--text-light); font-size: 0.95rem;">Financial Engineering Guest Lecturer — Statistical Inference in Finance</p>
    <p style="color: var(--text-light); font-size: 0.85rem; margin-top: 8px;">
      📧 fred.egbuedike@email.com &nbsp;|&nbsp; 📱 +86 138 0000 0000 &nbsp;|&nbsp; 📍 Chengdu, China
    </p>
  </div>

  <h3 style="color: var(--primary); margin-bottom: 12px;">Professional Summary</h3>
  <p style="margin-bottom: 16px; line-height: 1.6;">
    Quantitative educator and technical trainer with 8+ years of university-level teaching and corporate training experience. M.Sc. in Information Security from Southwest Jiaotong University with strong computational and quantitative foundations. Proven track record delivering English-medium instruction to Chinese professionals and researchers at Fortune Global 500 companies and national research institutions. Experienced speaker at international blockchain and fintech conferences (2019–2021). Prepared to deliver a rigorous 64-hour <strong>Statistical Inference in Finance</strong> course combining mathematical theory with practical financial applications.
  </p>

  <h3 style="color: var(--primary); margin-bottom: 12px;">Teaching Experience</h3>

  <div style="margin-bottom: 16px; padding-left: 16px; border-left: 3px solid var(--primary);">
    <div style="display: flex; justify-content: space-between; align-items: baseline;">
      <div>
        <strong>English Instructor — Academic Communication & Presentation Training</strong><br>
        <span style="color: var(--primary); font-weight: 600;">Chinese Academy of Sciences (中国科学院, CAS)</span>
      </div>
      <span style="color: var(--text-light); font-size: 0.85rem;">2013</span>
    </div>
    <ul style="margin: 4px 0 0 20px; font-size: 0.9rem; color: var(--text-light);">
      <li>Trained 60+ PhD students and researchers in academic English communication and presentation skills</li>
      <li>Prepared researchers for international publication and conference presentation</li>
    </ul>
  </div>

  <div style="margin-bottom: 16px; padding-left: 16px; border-left: 3px solid var(--primary);">
    <div style="display: flex; justify-content: space-between; align-items: baseline;">
      <div>
        <strong>Language Consultant — Corporate English Training</strong><br>
        <span style="color: var(--primary); font-weight: 600;">Midea Group (美的集团) — Fortune Global 500</span>
      </div>
      <span style="color: var(--text-light); font-size: 0.85rem;">2024–2025</span>
    </div>
    <ul style="margin: 4px 0 0 20px; font-size: 0.9rem; color: var(--text-light);">
      <li>Designed and delivered English-medium technical training for Chinese professionals across legal, finance, R&D, and sales departments</li>
      <li>Created course materials, exercises, and assessments for adult learners</li>
    </ul>
  </div>

  <div style="margin-bottom: 16px; padding-left: 16px; border-left: 3px solid var(--primary);">
    <div style="display: flex; justify-content: space-between; align-items: baseline;">
      <div>
        <strong>Language Consultant — Corporate English Training</strong><br>
        <span style="color: var(--primary); font-weight: 600;">Hisense Group (海信集团) — Fortune Global 500</span>
      </div>
      <span style="color: var(--text-light); font-size: 0.85rem;">2024–2025</span>
    </div>
    <ul style="margin: 4px 0 0 20px; font-size: 0.9rem; color: var(--text-light);">
      <li>Developed and delivered English-medium corporate training for research, finance, legal, and sales departments</li>
      <li>Created lesson plans, materials, and assessments for professional learners</li>
    </ul>
  </div>

  <div style="margin-bottom: 16px; padding-left: 16px; border-left: 3px solid var(--primary);">
    <div style="display: flex; justify-content: space-between; align-items: baseline;">
      <div>
        <strong>Executive Lecturer</strong><br>
        <span style="color: var(--primary); font-weight: 600;">NetEase (网易)</span>
      </div>
      <span style="color: var(--text-light); font-size: 0.85rem;">2019–2020</span>
    </div>
    <ul style="margin: 4px 0 0 20px; font-size: 0.9rem; color: var(--text-light);">
      <li>Delivered executive-level English training for senior professionals</li>
    </ul>
  </div>

  <div style="margin-bottom: 16px; padding-left: 16px; border-left: 3px solid var(--primary);">
    <div style="display: flex; justify-content: space-between; align-items: baseline;">
      <div>
        <strong>ESL Instructor</strong><br>
        <span style="color: var(--primary); font-weight: 600;">Private Language Training Schools</span>
      </div>
      <span style="color: var(--text-light); font-size: 0.85rem;">2014–2018</span>
    </div>
    <ul style="margin: 4px 0 0 20px; font-size: 0.9rem; color: var(--text-light);">
      <li>4 years full-time English instruction to diverse learner populations</li>
      <li>Developed curriculum, lesson plans, and assessment materials</li>
    </ul>
  </div>

  <h3 style="color: var(--primary); margin-bottom: 12px;">Conference Speaking</h3>
  <div style="margin-bottom: 16px; padding-left: 16px; border-left: 3px solid var(--primary);">
    <div style="display: flex; justify-content: space-between; align-items: baseline;">
      <div>
        <strong>Blockchain & Fintech Conference Speaker</strong><br>
        <span style="color: var(--primary); font-weight: 600;">International Conferences</span>
      </div>
      <span style="color: var(--text-light); font-size: 0.85rem;">2019–2021</span>
    </div>
    <ul style="margin: 4px 0 0 20px; font-size: 0.9rem; color: var(--text-light);">
      <li>Delivered presentations on blockchain technology and financial applications</li>
      <li>Bridged technical and financial domains for diverse professional audiences</li>
    </ul>
  </div>

  <h3 style="color: var(--primary); margin-bottom: 12px;">Education</h3>
  <div style="margin-bottom: 12px; padding-left: 16px; border-left: 3px solid var(--primary);">
    <div style="display: flex; justify-content: space-between; align-items: baseline;">
      <div>
        <strong>M.Sc. — Information Security</strong><br>
        <span style="color: var(--text-light);">Southwest Jiaotong University (西南交通大学), Chengdu</span>
      </div>
      <span style="color: var(--text-light); font-size: 0.85rem;">2013</span>
    </div>
  </div>
  <div style="margin-bottom: 12px; padding-left: 16px; border-left: 3px solid var(--primary);">
    <div style="display: flex; justify-content: space-between; align-items: baseline;">
      <div>
        <strong>B.Sc. — Computer Science</strong><br>
        <span style="color: var(--text-light);">University of Nigeria</span>
      </div>
      <span style="color: var(--text-light); font-size: 0.85rem;">2010</span>
    </div>
  </div>

  <h3 style="color: var(--primary); margin-bottom: 12px;">Certifications & Skills</h3>
  <p style="margin-bottom: 16px; line-height: 1.6;">
    <strong>TEFL Certified (2019)</strong> · Native English Speaker · Conversational Chinese · Python & Data Analysis · Quantitative Methods · Curriculum Design · Classroom Management · Interactive Teaching
  </p>

  <h3 style="color: var(--primary); margin-bottom: 12px;">Course Ready to Teach</h3>
  <p style="margin-bottom: 16px; line-height: 1.6;">
    <strong>Statistical Inference in Finance</strong> — 64 teaching hours, 4-week intensive covering probability foundations, estimation theory, hypothesis testing, and regression analysis with financial applications. Full syllabus, materials, and assessments prepared.
  </p>

  <div style="margin-top: 24px; padding-top: 16px; border-top: 1px solid var(--border); text-align: center; font-size: 0.85rem; color: var(--text-light);">
    <strong>References available upon request.</strong> · Available to teach September 28 – October 23 in Changchun
  </div>
</div>
